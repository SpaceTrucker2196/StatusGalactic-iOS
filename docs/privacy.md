...
> accurate to the v0.2 release build and match the in-app and App Store
> privacy disclosures.
...
crash reporters, and no third-party SDKs. The app developer
...
- **No Contacts, Calendar, Photos, Microphone, Camera, HealthKit,
  Motion, or Bluetooth access.**
...
3. **iCloud Backup.** Apple's standard device backup may include the
   app's `UserDefaults` if you have iCloud Backup enabled in iOS
   Settings. That is the operating system doing this, not Spacetrucker
   Galactic — the app never uploads anything to iCloud directly.
