...
2026-05-20 — polish pass — SunStrip now exposes an accessibilityLabel that reads sunrise / sunset / current twilight phase to VoiceOver. BriefView gains success/error sensoryFeedback haptics on refresh. When user selects a callsign without an aprs.fi key, ViewModel surfaces a clear "Set your API key in Settings" message instead of the raw HTTP error. BriefView gains a guided empty state with "Allow Location" / "Open iOS Settings" buttons when permission is missing.
